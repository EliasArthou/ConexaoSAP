...
class Gradus:
    ...
    def verificastatusconexao(self):
        try:
            conec = Banco()
            retorno = conec.consultar(f"""
                                        SELECT CASE WHEN dataacesso IS NULL OR dataacesso < DATEADD(ms, -ISNULL([timeout], -1), GETDATE()) THEN 0 
                                                ELSE CASE WHEN DATEDIFF(ms, dataacesso, GETDATE()) < ISNULL([timeout], -1) THEN 1 
                                                ELSE 0 END END AS ConexaoAtiva
                                        FROM [DB_PLAN_CTRO].[PLAN_CTRO].[Acesso API GRADUS]
                                        WHERE usuario = '{self.usuario}' AND Tipo = '{self.tipo}'""")

            if not retorno.empty:
                linha = retorno.iloc[0]
                resposta = (linha['ConexaoAtiva'] == 1)
            else:
                resposta = 0
                            ...
                            match response.status_code:
                                case 200:
                                    ...
                                    for column in dfresultado.columns:
                                        if len(str(column)) > 2:
                                            if (column[-2:] == "At" or column[-4:] == "Date" or column[
                                                                                                :4] == "date") and pd.api.types.is_numeric_dtype(
                                                dfresultado[column]):
